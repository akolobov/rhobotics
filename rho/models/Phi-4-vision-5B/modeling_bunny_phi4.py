"""
Minimal self-contained Bunny-Phi4 model implementation.

This module provides:
- BunnyPhi4Config: Configuration class
- BunnyPhi4ForCausalLM: Main vision-language model
...
class ModelArguments:
    ...
    pretrain_mm_mlp_adapter: str | None = None
    use_s2: bool = False
...
class RhoImageProcessorNoUpscale(nn.Module):
    """
    GPU-native NaFlex image processor that accepts (C, H, W) float tensors
    in [0, 1] directly, avoiding CPU round-trips through PIL/numpy.

    Drop-in replacement for Siglip2ImageProcessorNoUpscale when images are
    already tensors (as produced by the rho data pipeline).
    """
    ...
    def __init__(
        ...
        self.register_buffer(
            "_image_mean", torch.tensor(image_mean, dtype=torch.float32).view(3, 1, 1)
        )
        self.register_buffer(
            "_image_std", torch.tensor(image_std, dtype=torch.float32).view(3, 1, 1)
        )

    @classmethod
    def from_siglip2_processor(cls, processor: "Siglip2ImageProcessorNoUpscale"):
        """Create from an existing Siglip2ImageProcessorNoUpscale instance."""
        return cls(
    ...
    def _compute_target_size(self, height: int, width: int) -> tuple[int, int]:
        """Return (H, W) that satisfies the min/max patch-count constraints."""
        p = self.patch_size
        num_patches = max((height // p) * (width // p), 1)

        if num_patches < self.min_num_patches:
            target_patches = self.min_num_patches
        elif num_patches > self.max_num_patches:
            target_patches = self.max_num_patches
        else:
            target_patches = num_patches

        return siglip2_ips.get_image_size_for_max_num_patches(
            image_height=height,
            image_width=width,
            patch_size=p,
            max_num_patches=target_patches,
        )

    def _extract_patches(self, image: torch.Tensor) -> torch.Tensor:
        """Convert a (C, H, W) tensor into (num_patches, patch_dim) patches."""
        C, H, W = image.shape
        p = self.patch_size
        nph, npw = H // p, W // p
        # (C, nph, p, npw, p) → (nph, npw, p, p, C) → (nph*npw, p*p*C)
    ...
    def _pad_patches(
        ...
        """Pad to max_num_patches; return (padded_patches, attention_mask)."""
        n = patches.shape[0]
    ...
    def __call__(
        self, images: list[torch.Tensor], return_tensors: str = "pt"
    ) -> BatchFeature:
        """
        Process a list of (C, H, W) float tensors in [0, 1].

        Args:
            images: List of (C, H, W) tensors, values in [0, 1].
            return_tensors: Ignored (always returns PyTorch tensors).

        Returns:
            BatchFeature with keys pixel_values, pixel_attention_mask,
            spatial_shapes — same contract as Siglip2ImageProcessorNoUpscale.
        """
        all_patches = []
        all_masks = []
        all_shapes = []
        ...
        for image in images:
            ...
            # Resize when the current dimensions don't match the target
...
class Siglip2VisionTower(nn.Module):
    ...
    def load_model(self, skip_weights: bool = False):
        ...
        self.image_processor = Siglip2ImageProcessorNoUpscale.from_pretrained(
            self.vision_tower_name,
            local_files_only=self.local_files_only,
            cache_dir=self.hf_hub_cache_dir,
...
class BunnyPhi4Config(Phi3Config):
    """Configuration for Bunny-Phi4 model."""
...
class BunnyPhi4Model(Phi3Model):
    """Phi3 model with vision tower and projector."""
    config_class = BunnyPhi4Config

    def __init__(self, config: BunnyPhi4Config):
...
class BunnyPhi4ForCausalLM(Phi3ForCausalLM):
    """Bunny-Phi4 model for causal language modeling with vision support."""
    config_class = BunnyPhi4Config
    ...
    def __init__(self, config: BunnyPhi4Config):
        ...
        self.model = BunnyPhi4Model(config)
...
AutoConfig.register("bunny-phi3", BunnyPhi4Config)
AutoModelForCausalLM.register(BunnyPhi4Config, BunnyPhi4ForCausalLM)
